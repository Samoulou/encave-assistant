...
Version 1.1 — exemples à implémenter et valider ; aucune route de production n’existe encore dans le POC.
...
Ce sont des noms proposés, pas des secrets ou comptes à réutiliser. Les connexions et refresh tokens sont propres aux caves, chiffrés et stockés côté serveur. Les IDs de tenant interne et de tenant Microsoft sont différents. Les variables d’environnement configurent l’application fournisseur et ses secrets par environnement ; elles ne contiennent pas une boîte ou un calendrier propres à Julien ou à une cave. Chaque client configure sa connexion dans l’application. Les contrats détaillés figurent dans [connecteurs et onboarding](10-connecteurs-et-onboarding.md).
