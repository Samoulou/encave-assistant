...
Le catalogue et les règles structurées sont prioritaires. Pour le périmètre initial, des FAQ courtes validées suffisent ; une base vectorielle n’est pas nécessaire par défaut. Si des documents sont ajoutés, indexer leur version, leur cave, leur statut approuvé et leur date d’expiration. Les pièces jointes entrantes passent par des limites, une analyse et une autorisation de traitement adaptées.
...
### Jeu d’évaluation
Préparer un jeu versionné d’au moins 100 demandes de test, anonymisées ou synthétiques, comprenant 20 cas adverses ou ambigus. Séparer les exemples utilisés pour améliorer les consignes et ceux de recette. Les retours facultatifs de Sam, des essais et des clients enrichissent le jeu de régression après anonymisation lorsqu’ils contiennent des données réelles. Aucune contribution de Julien n’est requise pour constituer et évaluer le corpus.
...
Utiliser Outlook comme logiciel ne prouve pas qu’il s’agit d’une boîte Exchange Online. Identifier le fournisseur, le type de boîte, le calendrier, son propriétaire, les délégations et les personnes qui y écrivent. Le connecteur vérifie les capacités de chaque connexion au lieu de supposer une configuration client. La première offre prend en charge les comptes professionnels Exchange Online du cloud commercial ; les comptes personnels Outlook.com, IMAP et Exchange sur site ne sont pas annoncés comme pris en charge au lancement. [S1]

Le fournisseur est un adaptateur commun à toutes les caves. Une application Entra multitenant gérée par EnCave Assistant permet à chaque organisation compatible de connecter son propre compte ; les données, autorisations et secrets restent rattachés à une connexion vérifiée de la cave. Le compte Microsoft n’est pas l’identité métier de la cave. [S21] Voir [le contrat et l’onboarding générique](10-connecteurs-et-onboarding.md).

### Deux chemins d’intégration à distinguer
**Boîte professionnelle propre et calendrier principal :** profil standard initial, répété sans changement de code pour chaque cave. OAuth délégué, accès minimal nécessaire à la lecture des messages, à l’envoi et à l’écriture du calendrier. Les scopes envisagés sont Mail.Read, Mail.Send, Calendars.ReadWrite, offline_access et les scopes d’identité utiles. La politique du tenant peut exiger un consentement administrateur. [S2]
...
### Faisabilité et preuves du connecteur
Le cadrage peut avancer sur contrats et fixtures. Avant activation commerciale du profil Microsoft, exécuter sur deux organisations Microsoft indépendantes, avec comptes autorisés contrôlés par Sam : connexion depuis l’interface, lecture d’une demande fictive, détection de sa réception, lecture d’occupation, création et rapprochement d’événement de test, envoi à une adresse de test autorisée, renouvellement d’accès, révocation puis reconnexion. Prouver le refus d’accès croisé et l’absence de changement de code ou de déploiement lors de la seconde connexion. Conserver séparément les résultats simulés et réels ; cette documentation ne fournit ni accès ni preuve déjà exécutée.

Si les accès ne sont pas disponibles ou si ce test échoue, poursuivre les lots indépendants avec saisie/formulaire et agenda interne. Le profil concerné reste non activable tant que ses preuves manquent. Ne pas présenter cette solution provisoire comme une intégration Microsoft opérationnelle.
