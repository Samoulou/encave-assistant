...
- Sam porte les décisions produit, les priorités, les hypothèses et la politique de livraison ; le développement ne dépend d’aucun prospect. La cave administre ses données et autorise ses engagements clients.
- Première intégration : connecteur générique Microsoft 365 pour comptes professionnels Exchange Online, connecté par chaque cave via OAuth et configuration, sans code spécifique ni redéploiement. Premières entrées : e-mail, formulaire du site et saisie manuelle après un appel. Le téléphone n’est pas décroché par une IA.
...
**État au 7 septembre 2026 :** un POC de parcours est déployé. Son moteur applique certaines règles de disponibilité, mais l’IA, Microsoft 365, le stockage serveur et les canaux réels restent à construire. Julien est un interlocuteur de découverte ; aucun engagement de pilote ni achat n’a été confirmé. Son retour est facultatif et ne constitue pas une condition de développement ou de lancement.

**Version :** 1.1 — mise à jour autorisée par Sam : gouvernance indépendante des prospects et onboarding générique. Les anciennes propositions de base partagée avec EnCave sont abandonnées. Les durées et prix présents dans le POC sont fictifs.
...
### Mesure facultative avant et pendant un essai client
Sur un échantillon de demandes réelles anonymisées, relever le canal, le type, le temps actif de traitement, les échanges, le résultat et les corrections. Mesurer ensuite les mêmes indicateurs avec le produit. Comparer des demandes de complexité similaire ; distinguer le délai d’attente du client et le travail de la cave. Le nombre de réponses produites ne suffit pas à prouver la valeur. À défaut de données réelles autorisées, développer et tester sur des jeux synthétiques ; la preuve de valeur commerciale reste alors à établir, sans bloquer la livraison technique décidée par Sam.

## 03 · Périmètre par version et exclusions

| Capacité | Périmètre minimal de test | Cible commerciale à sélectionner par Sam |
|---|---|---|
...
| E-mail Microsoft 365 | Adaptateur commun, deux caves de test, simulateur explicite | OAuth par cave, deux organisations Microsoft testées, surveillance et reprise |
...
Sam décide des ajouts, du périmètre commercial et des estimations. La réservation après devis et le canal sortant du profil sans Microsoft sont des lots identifiés. Le noyau peut progresser avec formulaire, saisie et agenda interne ; seule une capacité effectivement testée peut être vendue comme disponible.
...
5. Depuis « Connexions », choisir Microsoft 365, connecter son compte professionnel et sélectionner la boîte et les calendriers autorisés. Afficher les permissions et capacités obtenues ; tester sans données réelles, puis activer cette seule connexion. Aucun développeur ne renseigne un identifiant client dans le code. Voir le [parcours de connexion](10-connecteurs-et-onboarding.md).
...
### Données nécessaires à l’activation d’une cave
Catalogue approuvé par l’administrateur de la cave et propriétaire de chaque tarif ; demandes synthétiques de recette (exemples réels anonymisés facultatifs) ; structure des calendriers si connectés ; règles de prix et de capacité ; durées tampon ; conditions d’annulation ; personnes qui valident les exceptions ; politique d’accès et de conservation.

Le dossier de mise en service conserve la version des règles, les tests réussis, la date d’activation et le nom du responsable. La simple réussite de la connexion OAuth n’équivaut pas à une cave prête pour les réservations. Une configuration incomplète suspend uniquement les actions de cette cave qui en dépendent. Sam conserve la décision de livraison du produit ; il n’autorise pas à la place du client l’accès à ses comptes.
