...
export async function identityRuntime(file: string) {
  const config = JSON.parse(await readFile(file, 'utf8')) as { oidc: OidcSettings; database: pg.PoolConfig; exports?: { retentionSeconds: number }; microsoft?: MicrosoftSettings };
  ...
  try {
    ...
    return { handler: createIdentityHandler(new IdentityStore(pool), config.oidc, {...(config.exports?{exportRetentionSeconds:config.exports.retentionSeconds}:{}),...(config.microsoft?{microsoft:config.microsoft}:{})}), stop: () => pool.end() };
